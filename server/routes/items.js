...
router.post('/sold', async (req, res) => {
    const { name, user_id } = req.body
    try {
        const { firstName, lastName } = await User.findOneAndUpdate({ _id: user_id }, { $push: { scannedItems: name, lastScan: Date.now() } })
        const item = await Item.findOneAndUpdate({ name }, { $inc: { items_left: -1 }, firstName, lastName })
...
router.post('/edit', async (req, res) => {
    const { name, id, totalItems } = (req.body)
    try {
        const response = await Item.findByIdAndUpdate(id, { totalItems, items_left: totalItems }, { new: true })
    ...
    try {
        ...
        data.forEach(user => {
            worksheet.addRow({
                ...
                scannedItems: `(${user.scannedItems.length})` + user.scannedItems.join(', '),
        ...
        const fileMetadata = {
            name: 'users.xlsx',
            parents: ['1yvI5NRlJKD03meK3PdTidoFSsruQfrpD'],
        }
        const media = {
