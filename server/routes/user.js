...
router.post('/', async (req, res) => {
    ...
    const { email, firstName, lastName, password, username, user_type, image } = req.body;
    ...
    try {
        let data = new User({
            email, firstName, lastName, password, username, user_type, image
        })
...
router.post('/update', async (req, res) => {
    ...
    try {
        const data = await User.findOneAndUpdate({ user_type: 'admin' }, { image: req.body.image })
        console.log(data)
...
router.post('/login', async (req, res) => {
    ...
    if (!email || !password) {
        return res.status(404).json({ status: "Failed", message: "Enter Valid Info" })
    }
