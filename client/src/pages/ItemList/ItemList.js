...
const ItemList = () => {
    ...
    return (
        <div>
            ...
            {userRole === 'admin' && <button className='update_btn' onClick={() => navigate('/editItems')}>Edit Stock</button>}

            <div style={{ margin: 20 }} className='search_bar'>
                <input value={search} onChange={handleSearch} type="text" placeholder='Search...' />
            </div>
            <div>
                <Table bordered striped align='center' style={{ marginTop: '50px' }} width='60%' border="1">
                    <thead>
                    ...
                    <tbody >
                        {items.map((item, index) => (
                            ...
                            <tr key={item._id}>
                                ...
                                {
                                    item.items_left === 0 ?
                                        <td style={{ backgroundColor: 'red' }} align='center' > {item.items_left}</td>
                                        :
                                        item.items_left < 5 ?
                                            <td style={{ backgroundColor: 'yellow' }} align='center' > {item.items_left}</td>
                                            :
